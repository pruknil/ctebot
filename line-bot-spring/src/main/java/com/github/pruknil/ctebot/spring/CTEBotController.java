...
public class CTEBotController {
    ...
    public void handleMemberLeft(MemberLeftEvent event) {
        log.info("Got memberLeft message: {}",
                event.getLeft().getMembers().stream().map(Source::getUserId).collect(Collectors.joining(",")));
    }
    ...
    private void push(@NonNull String replyToken, @NonNull List<Message> messages) {
        try {
            BotApiResponse apiResponse = lineMessagingClient.pushMessage(new PushMessage(replyToken, messages))
                    .get();
            log.info("Sent messages: {}", apiResponse);
        } catch (InterruptedException | ExecutionException e) {
            throw new RuntimeException(e);
    ...
    private void handleTextContent(String replyToken, MessageEvent<TextMessageContent> event,
        ...
        if ("#batchple".equalsIgnoreCase(text)) {
            handleHeavyContent(replyToken, event.getMessage().getId(), responseBody -> {
                reply(event.getReplyToken(), new ImageMessage(createUri("/static/batchple.jpg"), createUri("/static/batchple.jpg")));
            });
        } else {
    ...
    private static DownloadedContent createTempFile(String ext) {
        ...
        Path tempFile = CTEBotApplication.downloadedContentDir.resolve(fileName);
